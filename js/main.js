  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function typeWriter(element, text, i, callback) {
      if (i < text.length) {
        ...
        }, 150); // Adjust the speed by changing the timeout value
      } else {
        callback();
      }
    }
            ...
            for (let i = 0; i < jobTitles.length; i++) {
              setTimeout(function() {
                ...
                typeWriter(currentElement, jobTitles[i], 0, function() {
                  ...
                  }, 2500); // Adjust the pause before typing the next job title
                });
              }, i * 3500); // Adjust the delay before typing each job title
            }
          } else {
